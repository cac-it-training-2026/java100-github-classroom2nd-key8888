...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (checkDuplication(ABKosanArray, ramdomInt)) {
				ABKosanArray[index] = ramdomInt;
				index++;
			}
			if (index > 4) {
				break;
		...
		//ここに値の入れ替え処理を記述する
		int oneIndex = findTargetIndex(ABKosanArray, 1);
		int twoIndex = findTargetIndex(ABKosanArray, 2);
		int threeIndex = findTargetIndex(ABKosanArray, 3);
		int fourIndex = findTargetIndex(ABKosanArray, 4);

		swap(ABKosanArray, oneIndex, threeIndex);
		swap(ABKosanArray, twoIndex, fourIndex);
